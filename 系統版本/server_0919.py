...
app = FastAPI(title="桃園圖書館中壢分館查書系統")
...
def load_default_books() -> List[str]:
    filepath = os.path.join(os.path.dirname(__file__), "default_books.txt")
...
def run_crawler_job(books: List[str], target_branch: str):
    ...
    broadcast_event({
        ...
        "message": f"開始檢索 {len(books)} 本書，目標館別：【{target_branch}分館】..."
    })
...
def export_csv():
    """匯出 UTF-8-BOM CSV 檔案供 Excel 正常開啟。"""
    output = io.StringIO()
    # 寫入 BOM
    output.write('\ufeff')
    ...
    # 標題列
    writer.writerow([
        "序號", "原始書目", "查詢結果書名", "中壢分館狀態", "索書號", "館藏地/室", "狀態說明", "條碼號", "圖書館連結"
    ])
    ...
    return Response(
        content=csv_data,
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=taoyuan_zhongli_books.csv"}
    )
