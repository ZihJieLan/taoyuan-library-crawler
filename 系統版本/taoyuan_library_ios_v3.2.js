...
/**
 * 桃園市立圖書館 - 在館書目智慧檢索 (iPhone Scriptable 專用版 v3.2)
 * 
 * v3.2 更新重點：
 * 1. 【Google Keep 清單智慧支援】：
 *    - 貼上清單時，自動識別並截斷 `[x]`、`[X]` 或 `[✓]` 已讀標記及其後方所有書目。
 *    - 自動清除待讀書目前方的 `[ ] `、`- [ ] ` 核取方塊符號。
 *    - 保留空白行截斷與「閱讀書目」標題過濾功能。
 * 2. 【剪貼簿一鍵貼上】：輸入框上方新增「📋 貼上剪貼簿」按鈕，免手動長按尋找游標。
 * 3. 【iOS 捷徑/分享面板直連】：支援由 iOS「分享」面板直接將筆記文字傳入腳本執行。
 * 4. 【版本標示】：介面與檔名全面升級標記為 v3.2。
 */
...
<body>
  <div class="header">
    <h1>📚 桃園圖書館在館查書 v3.2</h1>
    <p>純 iPhone 本機執行 · 官方高速直連 · v3.2</p>
  </div>
