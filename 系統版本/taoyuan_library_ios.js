...
/**
 * 桃園市立圖書館 - 在館書目智慧檢索 (iPhone Scriptable 專用版 v3.3)
 * 
 * v3.3 更新重點：
 * 1. 【修復一鍵複製按鈕】：
 *    - 徹底移除在 iOS WKWebView 環境下會引發 TypeError 的 navigator.clipboard.writeText。
 *    - 改用 Scriptable 原生系統級 Pasteboard.copy() 搭配 document.execCommand('copy') 雙重機制，
 *      保證任何 iOS 環境下點擊「一鍵複製」皆能 100% 成功複製並附帶震動與 Toast 提示。
 * 2. 【版本升級】：檔名與介面全面標示為 v3.3。
 */
...
<body>
  <div class="header">
    <h1>📚 桃園圖書館在館查書 v3.3</h1>
    <p>純 iPhone 本機執行 · 官方高速直連 · v3.3</p>
  </div>
  ...
  <script>
    ...
    // 智慧書單清理過濾器：過濾標題、清除 [ ] 符號，遇到 [x] 或空白行時自動截斷已讀書目
    function cleanBookInputText(text) {
      ...
      for (const rawLine of lines) {
        ...
        // 2. 遇到 [x]、[X]、[✓] 等已讀標記時，直接中斷讀取（截斷後方所有書目）
        if (/^[\\s\\-\\*]*\\[[xX✓v]\\]/i.test(line)) {
          truncated = true;
          break;
        }
        ...
        // 4. 當已有書目時，遇到空白行自動切斷（空白行後面全視為已讀書目）
        if (!line) {
          if (cleaned.length > 0) {
            truncated = true;
            break;
    ...
    bookInput.addEventListener('input', () => {
      ...
      if (truncated || bookInput.value.includes('[ ]')) {
        bookInput.value = cleaned.join('\\n');
        showToast(\`已過濾已讀書目（保留 \${cleaned.length} 本待讀）\`);
      }
    });
    ...
    btnCopy.addEventListener('click', () => {
      ...
      // 1. 本地透過 textarea 執行標準 execCommand 複製（相容所有 iOS WebView）
      try {
      ...
      // 2. 透過 Scriptable 原生 Pasteboard.copy() 進行雙重保險複製
      sendToScriptable('scriptable-copy', text);
      showToast(\`✅ 已複製 \${listToCopy.length} 本書單到剪貼簿！\`);
    });
